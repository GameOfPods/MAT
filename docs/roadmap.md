...
- [ ] Linking Sortformer pieces left a third speaker with one 2 s segment on an episode with two hosts. Either a real short voice (clip, jingle) or a piece that didn't match. Still happens after the linking fix: AMI IS1009a has 4 speakers, MAT finds 5 with 300 s pieces and 7 with 150 s pieces, leaving 52 s and 39 s of confusion plus 51 s and 70 s of false alarm. Try a lower `sortformer.link-threshold` on several AMI meetings, then consider merging speakers with very little time into the closest speaker by embedding, with a configurable minimum.
