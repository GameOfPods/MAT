...
**Next up, once the speaker library has been tested on the GPU box:** the words that end up without a speaker or with two. On a 3 hour episode that's 17 % of the transcript lines, which makes it the most visible flaw in what we hand out. It sits under stage 6, transcript quality.
...
- [ ] Document a few local model picks that fit on the 1080 Ti, with the context they can hold. **Needs a decision first: who holds the card and when.** 11 GB minus about 1 GB for the desktop doesn't fit an LLM next to the speech models. Measured peaks: whisper large-v3-turbo 4.4 GB, Parakeet 9.2 to 9.6 GB, Sortformer 5.1 GB at 300 s pieces (2.0 GB at 150 s), community-1 6.1 GB, DiariZen 6.8 GB. An 8B model at Q4 wants roughly 6 GB on top of that, so with Parakeet it never fits and with whisper it's tight.
  ...
  Things to weigh: send `keep_alive: 0` so Ollama unloads right after the summary; keep the summary the last GPU step (it already is, but the speaker library loads the embedding model after the naming); run the LLM on CPU and accept it being slow; or use a hosted API on the GPU box and keep the card for speech. Whatever we pick has to survive `MAT run` over a folder of episodes, where the order repeats per file.
...
- [ ] The library runs after the LLM naming, so an episode with only known voices still pays for one LLM call. Move the matching in front of the naming and only ask the LLM about the speakers that are left.
