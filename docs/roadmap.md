...
- [ ] Review and rework the summary pipeline, including the prompts. Known so far:
  ...
  - The system message says "You may use contextual information you know on the source material", and the model used it: it compared a scene to "dem Haka der Neuseeländer", which nobody in the episode said (they talked about Icelandic football rituals). Drop that sentence, a summary should stay inside the transcript.
...
- [ ] Speaker matching runs even when it can't match anything. With the default `identifier = pyannote` and no gold labels it decodes the whole file and concatenates every segment per speaker, and the identifier then returns None without looking at the audio. On a 3 hour episode that was 2:35 of the 30:30 total. Ask the identifier whether it has gold labels before building the audio.
