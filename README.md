...
- A Hugging Face login only for gated models. The defaults don't need one: WeSpeaker (speaker names, linking of audio pieces) is not gated. [pyannote community-1](https://huggingface.co/pyannote/speaker-diarization-community-1) (the `pyannote-diarization` backend) is: accept its terms on the model page, then run `hf auth login` or set `HF_TOKEN`.
